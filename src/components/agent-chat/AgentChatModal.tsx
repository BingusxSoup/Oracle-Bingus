...
function buildDemoReply(agentName: string, text: string): string {
  return `${agentName} (demo): Received "${text}". Oracle Bingus is unavailable, so this is a simulated response.`
}
          ...
          setMessages([
            {
              ...
              role: 'agent',
              text: 'Oracle Bingus is unavailable. Running in demo mode with simulated responses.',
              timestamp: Date.now(),
            },
          ])
        }
