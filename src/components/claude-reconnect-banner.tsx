...
async function probeClaudeHealth(): Promise<boolean> {
  ...
  // which works with both Oracle Bingus and OpenAI-compatible backends.
  try {
              ...
              void fetch('/api/start-claude', {
                ...
                .then(async (res) => {
                  ...
                  if (res.ok && data.ok) {
                    setMessage(
                      data.message ||
                        'Auto-restarting Oracle Bingus gateway…',
                    )
  ...
  async function handleStartAgent(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok || !payload.ok) {
        throw new Error(payload.error || 'Failed to start Oracle Bingus')
      }

      setMessage(
        payload.message === 'already running'
          ? 'Oracle Bingus is already running'
          : 'Starting Oracle Bingus…',
      )
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : 'Failed to start Oracle Bingus',
      )
    } finally {
      setIsStarting(false)
      await probeNowRef.current?.(true)
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold">
              {isDisconnected ? 'Oracle Bingus not connected' : 'Connected'}
            </p>
            {message ? (
